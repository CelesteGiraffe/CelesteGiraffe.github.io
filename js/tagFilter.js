  ...
  container.innerHTML = `
    ...
    <div class="tag-checkboxes-grouped" id="tag-checkboxes-container">
      ${disciplineSections.map(section => {
        const disciplineKey = section.key.startsWith('game') ? 'game' : 
          (section.key.startsWith('backend') || section.key.startsWith('systems') || section.key.startsWith('devops') ? 'backend' : 
          (section.key.startsWith('web') ? 'web' : 
          (section.key.startsWith('fullstack') ? 'fullstack' : 'default')));
        
        return `
        <section class="tag-discipline-section tag-discipline-section--${disciplineKey}" data-discipline="${escapeHtml(section.key)}">
          <h4 class="tag-discipline-title">
            <span class="discipline-pip discipline-pip--${disciplineKey}" aria-hidden="true"></span>
